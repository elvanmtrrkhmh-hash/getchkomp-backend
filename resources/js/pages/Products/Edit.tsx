...
export default function Edit({
    ...
    const initialOverview = Array.isArray(product.overview)
        ? product.overview.map((item: any) =>
              typeof item === 'object' && item !== null ? item.title || '' : item
          )
        : product.overview && typeof product.overview === 'string'
          ? JSON.parse(product.overview).map((item: any) =>
                typeof item === 'object' && item !== null ? item.title || '' : item
            )
          : [''];
    ...
    const { data, setData, post, transform, processing, errors } = useForm<Record<string, any>>({
        ...
        overview: initialOverview.length > 0 ? initialOverview : [''],
        ...
        specs: product.specs && typeof product.specs === 'object' && !Array.isArray(product.specs)
            ...
            : [{ label: '', value: '' }],
        options: product.options && typeof product.options === 'object' && !Array.isArray(product.options)
            ? Object.entries(product.options).map(([key, values]) => ({ key, values: Array.isArray(values) ? values : [] }))
            : [],
    });
    ...
    const [imagePreviews, setImagePreviews] = useState<string[]>(
        Array.isArray(product.images) ? product.images.map((img) => `/storage/${img}`) : []
    );
    ...
    const removeImage = (index: number) => {
        const currentImages = data.images as any[];
        const item = currentImages[index];
        if (typeof item === 'string') {
            const imgIndex = (product.images as string[]).indexOf(item);
            setImagePreviews((prev) => prev.filter((_, i) => i !== imgIndex));
        } else {
            const newFilesOnly = currentImages.filter((img) => typeof img !== 'string');
            const newFileIndex = newFilesOnly.indexOf(item);
            setNewImagePreviews((prev) => prev.filter((_, i) => i !== newFileIndex));
        }
        setData('images', currentImages.filter((_: any, i: number) => i !== index));
    };
                    ...
                    <div className="space-y-1">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="flex gap-1.5">
                                {product.isFeatured && (
                                    <Badge variant="secondary" className="bg-amber-100 text-amber-700 ring-1 ring-amber-400/30">
                                        <Star className="mr-1 size-3 fill-amber-500" />
                                        Featured
                                    </Badge>
                                )}
                                {product.isBestseller && (
                                    <Badge variant="secondary" className="bg-blue-100 text-blue-700 ring-1 ring-blue-400/30">
                                        Bestseller
                                    </Badge>
                                )}
                            </div>
                        </div>
                        <p className="text-muted-foreground mt-1 text-sm">Update information for product ID: {product.id}</p>
                    </div>
                                ...
                                <CardContent className="space-y-5 pt-5">
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="text-[10px] font-medium text-muted-foreground">{imagePreviews.length + newImagePreviews.length} images</span>
                                        </div>
                                        <div className="flex flex-wrap gap-2">
                                            ...
                                            {imagePreviews.map((preview, index) => {
                                                const dataIndex = data.images.indexOf(product.images?.[index]);
                                                return (
                                                    <div key={`old-${index}`} className="group relative size-14 overflow-hidden rounded-lg border bg-muted ring-1 ring-border/50">
                                                        <img src={preview} alt="" className="size-full object-cover transition group-hover:scale-110" />
                                                        <button
                                                            type="button"
                                                            onClick={() => removeImage(dataIndex)}
                                            ...
                                            {newImagePreviews.map((preview, index) => {
                                                const filesOnly = data.images.filter((img) => typeof img !== 'string');
                                                const dataIndex = data.images.indexOf(filesOnly[index]);
                                                return (
                                                    <div key={`new-${index}`} className="group relative size-14 overflow-hidden rounded-lg border border-primary/40 bg-primary/5 ring-1 ring-primary/20">
                        ...
                        <Card className="overflow-hidden border-none shadow-sm ring-1 ring-border/60">
                            ...
                            <CardContent className="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-2">
                                ...
                                <div className="space-y-4 lg:border-r lg:pr-6">
                                    <div className="flex items-center justify-between">
                                        <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground line-clamp-1">Key Features</Label>
                                        <Button
                                            type="button"
                                            variant="ghost"
                                            size="sm"
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <Card className="overflow-hidden border-none shadow-sm ring-1 ring-border/60">
                            ...
                            <CardContent className="space-y-4 pt-5">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="rating" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Rating Display</Label>
                        ...
                        <div className="sticky top-6 pt-4 space-y-4">
                            ...
                            <p className="text-[10px] text-muted-foreground text-center px-4 leading-relaxed">
                                Ensure all details are correct. Updating will affect storefront data immediately.
                            </p>
                        </div>
                    </div>
                </form>
